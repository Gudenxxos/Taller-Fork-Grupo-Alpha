...
void multiMatrix(size_t *mA, size_t *mB, size_t *mC, int D) {
    ...
    #pragma omp parallel private(Suma, pA, pB)  // Se establece una directiva al compilador con OpenMP para que
						// Cada hilo utilizado tenga las variables Suma, pA y pB privadas
    {
        #pragma omp for  // Se establece una directiva al compilador para que cada hilo realice el siguiente ciclo
